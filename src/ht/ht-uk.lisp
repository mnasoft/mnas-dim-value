...
Пример использования:
  (gethash \"M\" *m-coeff-ru*) => 1000000
  (gethash \"m\" *m-coeff-ru*) => 1/1000000")
...
(defun populate-nm->vl (m-coeff nm->value)
  (unless *nm-value-loaded*
    (loop :for nd :in *nd-list* :do 
      (loop :for i :in nd :do
        (add-multiplid-values i m-coeff nm->value)))
    ...
    (loop :for nd :in *nd-list* :do
         (loop :for i :in nd :do
           (setf
            ...
            (<vd>-dims (<nd>-value i)))))
    (remhash "г" *unit-symbol->dim*)
    (setf
     (gethash "кг" *unit-symbol->dim*)
     '(0 1 0 0 0 0 0 0 0))
    (print-hash-table *unit-symbol->dim*)
...
#+nil 
(repopulate *table-7* *m-coeff* *nm->value*)
